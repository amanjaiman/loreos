...
The generated multi-resolution `lore.ico` is also copied into the packaged resources of the
**Windows** build and assigned directly to every `BrowserWindow`. The main process sets
`com.squirrel.Lore.Lore`, matching the AppUserModelID on Squirrel's installed shortcut;
together these prevent Windows from grouping the window under Electron's default taskbar
icon. Electron only decodes ICO on Windows, so the other builds package `lore.png` instead,
and a build carrying neither leaves the icon to the window manager rather than handing
`BrowserWindow` an image it can't read.
