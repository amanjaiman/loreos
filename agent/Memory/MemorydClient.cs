...
public sealed class MemorydClient : IMemoryService
{
    ...
    public MemorydClient(HttpClient httpClient)
    {
    ...
    public Task<IReadOnlyList<MemoryRecord>> GetRecentAsync(
        ...
        int count = 20,
        CancellationToken cancellationToken = default) =>
        ListAsync(userId, count, "get_recent", cancellationToken);

    public Task<IReadOnlyList<MemoryRecord>> GetAllAsync(
        string userId = "default",
        CancellationToken cancellationToken = default) =>
        ListAsync(userId, int.MaxValue, "get_all", cancellationToken);
    ...
    {
        Uri uri = Relative($"memories?user_id={Uri.EscapeDataString(userId)}&limit={limit}");
